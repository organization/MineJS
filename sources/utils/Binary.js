...
module.exports = {
    onLoad: ()=>{
        ...
        minejs.utils.Binary = class Binary{
            ...
            static readUUID(bytes){ return require('node-uuid').unparse(bytes); }
            static writeUUID(uuid){ return require('node-uuid').parse(uuid); }
            ...
            static readBool(b){ return b == 0; }
            static writeBool(b){ return (b ? 0x01 : 0x00); }
            static readSignedByte(b){ return b & 0xFF; }
            static writeByte(b){ return b; }
            static readShort(bytes){ ((bytes[0] & 0xFF) << 8) + (bytes[1] & 0xFF); }
            static readSignedShort(bytes){ return this.readShort(bytes); }
