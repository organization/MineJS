...
class SessionManager {
    ...
    streamEncapsulated(session, packet, flags) {
        ...
        for (let i = 0; i < id.length; ++i) {
            idBytes.push(id.charCodeAt(i));
        }
        let buffer = minejs.utils.Binary.appendBytes(
            minejs.raknet.RakNet.PACKET_ENCAPSULATED, [id.length() & 0xff],
            idBytes, [flags & 0xff],
    ...
    streamRAW(address, port, payload) {
        ...
        for (let i = 0; i < address.length; ++i) {
            addressByte.push(address.charCodeAt(i));
        }
        let buffer = minejs.utils.Binary.appendBytes(
            minejs.raknet.RakNet.PACKET_RAW, [(address.length() & 0xff)],
            addressByte,
    ...
    streamClose(identifier, reason) {
        ...
        for (let i = 0; i < identifier.length; ++i) {
            identifierByte.push(identifier.charCodeAt(i));
        }
        let reasonByte = [];
        for (let i = 0; i < reason.length; ++i) {
            reasonByte.push(reason.charCodeAt(i));
        }

        let buffer = minejs.utils.Binary.appendBytes(
            minejs.raknet.RakNet.PACKET_CLOSE_SESSION, [identifier.length() & 0xff],
            identifierByte, [reason.length() & 0xff],
            reasonByte
        );
        this.receiveStream(buffer);
    }
    ...
    streamInvalid(identifier) {
        ...
        let buffer = minejs.utils.Binary.appendBytes(
            minejs.raknet.RakNet.PACKET_INVALID_SESSION, [identifier.length() & 0xff],
            identifierByte
        );
        this.receiveStream(buffer);
    }
    ...
    streamOpen(session) {
        ...
        for (let i = 0; i < sessionAddress.length; ++i) {
            sessionAddressByte.push(sessionAddress.charCodeAt(i));
        }

        let buffer = minejs.utils.Binary.appendBytes(
            minejs.raknet.RakNet.PACKET_OPEN_SESSION, [identifier.length() & 0xff],
            identifierByte, [session.getAddress().length() & 0xff],
            sessionAddressByte,
    ...
    streamACK(identifier, identifierACK) {
        ...
        let buffer = minejs.utils.Binary.appendBytes(
            minejs.raknet.RakNet.PACKET_ACK_NOTIFICATION, [identifier.length() & 0xff],
            identifierByte,
    ...
    streamOption(name, value) {
        ...
        for (let i = 0; i < name.length; ++i) {
            nameByte.push(name.charCodeAt(i));
        }

        let valueByte = [];
        for (let i = 0; i < value.length; ++i) {
            valueByte.push(value.charCodeAt(i));
        }

        let buffer = minejs.utils.Binary.appendBytes(
            minejs.raknet.RakNet.PACKET_SET_OPTION, [name.length() & 0xff],
            nameByte,
            valueByte
        );
        this.receiveStream(buffer);
